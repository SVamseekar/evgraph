...
| 3 | Rule packs and reporters can be distributed as independent packages without violating the layered architecture. | ✅ Validated | Stage 3 — `evgraph-rules` extracted as its own package; three reporters (JSON/Markdown/SARIF) coexist on the same `Finding` list (`docs/ROADMAP.md` Stage 3). |
| 4 | A third-party rule pack can be discovered and affect `scan()` output purely by being installed, with zero changes to any `evgraph-*` package. | ✅ Validated | Stage 4 — real entry_point-based discovery; a scratch third-party package (`third-party-rule`) proved and cleanly reverted (`docs/ROADMAP.md` Stage 4). |
