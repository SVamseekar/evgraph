...
from evident.adapters.model_card import ModelCardAdapter, ModelCardArtifacts
...
def scan(model_card_path: str | Path, approval_path: str | Path, deployment_path: str | Path) -> Report:
    ...
    yet (that's future orchestration in `evident`, also out of APS-Core's scope).

    Every rule discovered via the "evident.rules" entry_point group (§discovery.py)
    ...
    being pip installed, with no change to evident's source.
    """
