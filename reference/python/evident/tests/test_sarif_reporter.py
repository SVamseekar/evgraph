...
from evident_core import EvidenceGraph, EvidenceLevel, EvidenceNode, Finding, Outcome

from evident.reporters.sarif_reporter import to_sarif_dict
...
def test_sarif_has_required_top_level_shape():
    ...
    assert sarif["runs"][0]["tool"]["driver"]["name"] == "evident"
...
def test_result_preserves_evident_specific_fields_in_properties():
    sarif = to_sarif_dict(make_graph(), [make_finding(Outcome.EXPECTATION_NOT_MET)])
    props = sarif["runs"][0]["results"][0]["properties"]
    assert props["evidentLevel"] == "STRUCTURAL"
    assert props["evidentOutcome"] == "EXPECTATION_NOT_MET"
