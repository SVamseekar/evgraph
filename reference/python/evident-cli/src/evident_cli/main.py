"""Thin CLI wrapper over the evident Python API.

Per docs/ARCHITECTURE.md's Developer First principle: this module contains no
logic that doesn't already exist in `evident`. Each subcommand does argument
parsing and output-format selection only; scanning and reporting are entirely
delegated to `evident.scan`/`evident.scan_dataset_manifest`/`Report`.
"""
