...
already present in `Finding.outcome` (RES §4.2) — Evident does not interpret
...
def _result_to_dict(finding: Finding) -> dict[str, Any]:
    return {
        ...
        "properties": {
            "evidentFindingId": finding.id,
            "evidentLevel": finding.level.name,
            "evidentOutcome": finding.outcome.value,
        ...
        "runs": [
            {
                "tool": {
                    "driver": {
                        "name": "evident",
                        "informationUri": "https://github.com/evident-org/evident",
                        "rules": rules,
                    }
                },
                "properties": {"evidentGraphId": graph.graph_id},
